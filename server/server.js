...
async function getChatByIdForUser(chatId, userId) {
  return dbGet(
    `
      SELECT id, user_id, title, selected_model, created_at
      FROM chats
...
app.post("/api/chat", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const selectedModel = chat.selected_model;
    const reply = await generateChatReply({
     message,
     history,
      language,
...
app.post("/api/multi-chat", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const modelOne = process.env.OLLAMA_MODEL_1 || process.env.OLLAMA_MODEL || "qwen3.5:4b";
    const modelTwo = process.env.OLLAMA_MODEL_2 || "llama3.2";
    ...
    if (!chat.title || chat.title === "New Chat") {
      const newTitle = message.length > 60 ? `${message.slice(0, 60).trim()}...` : message;
    ...
    return res.status(500).json({
      error: "Failed to generate multiple LLM responses."
    });
  }
});
...
app.post("/api/save-response", requireAuth, async (req, res) => {
  ...
  if (!chatId || !prompt || !responseText || !llmName) {
    return res.status(400).json({
  ...
  try {
    await saveSelectedResponse(dbRun, {
      userId,
      chatId,
      prompt,
      responseText,
